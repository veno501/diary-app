...
def diary(request, diary_id, diaryInstance):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            newEntry = Entry(title=form.cleaned_data['title'], parentDiary=diaryInstance)
            newEntry.save()
            return redirect('entry', diary_id=diaryInstance.uuid, entry_id=newEntry.uuid)
    else:
        form = EntryCreationForm()

    return render(request, 'diary/diary.html', 
        {'diary': diaryInstance, 'entry_list': entryList, 'form': form})
